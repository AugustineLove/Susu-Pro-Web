...
const Settings: React.FC = () => {
  ...
  const res = await fetch("http://127.0.0.1:5000/api/companies/toggle-2fa", {
    method: "POST",
  ...
  const data = { 
      ...
      console.log(data);
  try {
    const res = await fetch('http://127.0.0.1:5000/api/companies/change-password', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
const verifyOtp = async () => {
  ...
  const handleAddStaff = async () => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:5000/api/staff/create-agent', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
  ...
  try {
    const res = await fetch('http://127.0.0.1:5000/api/companies/update-profile', {
      method: 'POST',
      headers: {
  ...
  const handleSaveNotifications = async (logNotStat: boolean) => {
    try{
      const res = await fetch('http://127.0.0.1:5000/api/companies/login-notifications', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
  ...
  const handleSmsOrEmailNotifications = async (smsEnabled: boolean, emailEnabled: boolean, showWithdrawalAlerts: boolean, systemUpdates: boolean) => {
    try {
      const res = await fetch('http://127.0.0.1:5000/api/companies/sms-email-notifications', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
